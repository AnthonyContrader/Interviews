...
<body>
	...
	<div class="main">
				...
				<% for (QuestionDTO u : list) {%>
					<tr>
						...
						<%if(u.getRecruiterid()==user.getId() || user.getUsertype().equals("ADMIN")){%>
							<td><a class="edit" href=QuestionServlet?mode=read&update=true&id=<%=u.getId()%>>Edit</a>
							</td>
							<td><a class="delete" href=QuestionServlet?mode=delete&id=<%=u.getId()%>>Delete</a>
							</td>
						<%}else{%>
		...
		<div class="mainright">
			...
			<form action="QuestionServlet?mode=search" method="post">
			...
			<%if(!user.getUsertype().equals("USER")){%>
				<form action="QuestionServlet?mode=insert&recruiter=${user.getUsername()}&recruiterid=${user.getId()}
